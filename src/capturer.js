...
export async function capture({ frontendUrl, issueId, pageWidth, pageHeight }) {
  ...
  try {
    ...
    const url = `${frontendUrl}/preview/${issueId}`;
    await page.goto(url, { waitUntil: "networkidle2" });

    // 프론트엔드가 렌더링 완료 시 [data-snapshot-ready] 속성을 추가함
    // 이 속성이 나타날 때까지 최대 30초 대기
    const element = await page.waitForSelector("[data-snapshot-ready]", { timeout: 30000 });

    // 해당 요소만 캡처 — 페이지 전체가 아닌 이슈 콘텐츠 영역만 찍음
